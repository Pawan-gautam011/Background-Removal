    ...
    <>
      <nav className="bg-white/30 backdrop-blur-lg border border-white/30 shadow-md">
        <div className="flex justify-between p-3 items-center">
          <div className="flex items-center text-4xl  sm:text-left sm:text-3xl">
            ...
            { !isMenuOpen ? <IoMdMenu className="flex sm:hidden ml-5 items-center cursor-pointer" onClick={toggleMenu} /> :
              <RxCross2 className="flex sm:hidden ml-5 items-center cursor-pointer" onClick={toggleMenu}  />}
          </div>
        ...
        <div className={`${isMenuOpen ? 'flex' : 'hidden'} sm:hidden flex-col items-center bg-white/60 backdrop-blur-lg border-t border-white/30 shadow-md`}>
          <ul className="flex flex-col text-xl text-[#67737D] space-y-4 mt-4">
            <NavLink to="/remove" className='hover:bg-slate-300 p-2 hover:rounded-xl min-w-[150px] text-center'>Remove Background
